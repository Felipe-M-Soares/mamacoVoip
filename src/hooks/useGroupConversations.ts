...
export function useGroupConversations() {
  ...
  // Esse hook é chamado por vários componentes ao mesmo tempo (barra
  // lateral, layout principal, modais) — cada instância precisa do
  // seu próprio canal de tempo real com nome único, senão a segunda
  // tentativa de usar o mesmo nome de canal trava (o Supabase reusa
  // o canal já inscrito e não deixa adicionar listener depois).
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`group_conversation_membership:${instanceIdRef.current}`)
      .on(
        'postgres_changes',
